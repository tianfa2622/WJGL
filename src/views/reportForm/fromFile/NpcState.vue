        ...
        <el-row type="flex" class="tableBox" justify="center">
          ...
          <el-col :span="20">
            <el-table :data="tableData" border fit style="width: 100%" height="700">
              ...
              <el-table-column label="建议编号" prop="number" align="center" :resizable="false"> </el-table-column>
              <el-table-column label="建议标题" prop="title" align="center" :resizable="false"> </el-table-column>
              <el-table-column label="领衔委员" prop="leadingMember" align="center" :resizable="false"> </el-table-column>
              <el-table-column label="主办单位" prop="hostUnit" align="center" :resizable="false"> </el-table-column>
...
export default {
  ...
  methods: {
    ...
    InitialValue(time1, time2) {
      const value = `人大建议一览表（${time1}-${time2}）`
      return value
    },
    // 查询按钮
    async search(data) {
    ...
    exportExcel() {
      ...
      if (this.tableData.length > 0) {
        ...
        const url = `http://192.168.1.199:8086/api/FileReport/proposalChartExcel?registerStartDate=${startDate}&registerEndDate=${endDate}&fileType=9`
    ...
    // excel 数据导出
    exportToExcel() {
      const tHeader = ['序号', '建议编号', '建议标题', '领衔委员', '主办单位', '会办单位', '主办责任部门', '会办部门', '备注']
